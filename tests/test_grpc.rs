...
async fn test_grpc_mcp_flow() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for i in 0..20 {
            ...
            .await
        {
            Ok(channel) => {
                ...
                println!(
                    "Successfully connected to gRPC server after {}ms",
                    (i + 1) * 500
                );
                break;
            }
    ...
    assert!(
        !init_resp.result_json.is_empty(),
        "Initialize response should not be empty"
    );
    ...
    assert!(
        !list_resp.result_json.is_empty(),
    ...
    assert!(
        std::path::Path::new(&branch_db_file).exists(),
        "Branch file must exist"
    );
    ...
    assert!(
        rollback_resp.error_json.is_empty(),
